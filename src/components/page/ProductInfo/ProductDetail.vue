...
<script>
    ...
    export default {
        created(){
            ...
            if (this.type == "modify") {
                this.loading = true;
                this.$axios.get("/springbootbase/user/productInfo/detail", {params: {productId: this.$route.query.productId}}).then((res) => {
                    this.loading = false;
            ...
            onSubmit(formName) {
                ...
                self.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.loading = true;
                        if (this.type =="modify") {
                            this.$axios.post("/springbootbase/user/productInfo/update", this.form).then((res) => {
                                if (res.status == 200) {
                            ...
                            this.$axios.post("/springbootbase/user/productInfo/add", this.form).then((res) => {
                                if (res.status == 200) {
                                    this.$message.success('提交成功！');
                                    this.type = "modify";
                                    this.form.id = res.data.id;
